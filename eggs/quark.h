/* 
   Header file for quark, the easter egg text editor brought to you 
   by the ThinkGridWorldShell itself!
*/
...
#define QUARK_QUERY_LEN 256

/* Set up necessary buffers. */
#define ebuf_INIT {NULL,0}
#define FIND_RESTORE_HL do { \
    if (saved_hl) { \
        memcpy(quark.row[saved_hl_line].hl,saved_hl, quark.row[saved_hl_line].rsize); \
        saved_hl = NULL; \
    } \
} while (0)
...
typedef struct erow {
    ...
    unsigned char *hl;  /* Highlighting typquark*/
...
struct editorConfig {
    ...
    char statusmsg[80];             /* Status messagquark */
    time_t statusmsg_time;          /* Time associated with status messagquark */
    struct editorSyntax *syntax;    /* Syntax highlight. */
};

/* Editor configuration structure for the current session.*/
static struct editorConfig quark;
